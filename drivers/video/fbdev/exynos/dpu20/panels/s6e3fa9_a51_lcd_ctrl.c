...
#include "s6e3fa9_a51_param.h"
#include "dd.h"
#include "timenval.h"

#if defined(CONFIG_EXYNOS_DECON_MDNIE)
#include "mdnie.h"
#include "s6e3fa9_a51_mdnie.h"
#endif
...
#define DPUI_MODEL_NAME		"AMS646UJ01"
#endif
...
static int s6e3fa9_enteralpm(struct lcd_info *lcd)
{
	int ret = 0;
	union lpm_info lpm = {0, };
...
static int s6e3fa9_exitalpm(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3fa9_read_id(struct lcd_info *lcd)
{
...
static int s6e3fa9_read_coordinate(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3fa9_read_manufacture_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	memcpy(&lcd->manufacture_info, &buf[LDI_GPARA_MANUFACTURE_INFO], LDI_LEN_MANUFACTURE_INFO + LDI_LEN_MANUFACTURE_INFO_CELL_ID);

	return ret;
}

static int s6e3fa9_read_chip_id(struct lcd_info *lcd)
{
	int ret = 0;
...
static int panel_read_bit_info(struct lcd_info *lcd, u32 index, u8 *rxbuf)
{
	...
	if (print_tag) {
		...
		if (likely(decon)) {
			dev_info(&lcd->ld->dev, "==================================================\n");
			decon_abd_save_bit(&decon->abd, len * BITS_PER_BYTE, merged, print_new);
		}
		dev_info(&lcd->ld->dev, "==================================================\n");
...
static int s6e3fa9_read_rdnumpe(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3fa9_read_esderr(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3fa9_read_rddsdr(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3fa9_read_rddpm(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3fa9_read_rddsm(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3fa9_read_init_info(struct lcd_info *lcd)
{
	int ret = 0;
	...
	s6e3fa9_read_coordinate(lcd);
	s6e3fa9_read_manufacture_info(lcd);
	s6e3fa9_read_chip_id(lcd);
...
static int s6e3fa9_exit(struct lcd_info *lcd)
{
	int ret = 0;

	dev_info(&lcd->ld->dev, "%s\n", __func__);

	s6e3fa9_read_rddpm(lcd);
	s6e3fa9_read_rddsm(lcd);

#if defined(CONFIG_DISPLAY_USE_INFO)
	s6e3fa9_read_rdnumpe(lcd);

	DSI_WRITE(SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
	s6e3fa9_read_esderr(lcd);
	DSI_WRITE(SEQ_TEST_KEY_OFF_F0, ARRAY_SIZE(SEQ_TEST_KEY_OFF_F0));
#endif
...
static int s6e3fa9_displayon(struct lcd_info *lcd)
{
	int ret = 0;
...
static int s6e3fa9_init(struct lcd_info *lcd)
{
	int ret = 0;
...
#if defined(CONFIG_DISPLAY_USE_INFO)
	s6e3fa9_read_rddsdr(lcd);
#endif

	/* Common Setting */
	/* Testkey Enable */
	DSI_WRITE(SEQ_TEST_KEY_ON_F0, ARRAY_SIZE(SEQ_TEST_KEY_ON_F0));
	DSI_WRITE(SEQ_PAGE_ADDR_SET_2A, ARRAY_SIZE(SEQ_PAGE_ADDR_SET_2A));
	DSI_WRITE(SEQ_PAGE_ADDR_SET_2B, ARRAY_SIZE(SEQ_PAGE_ADDR_SET_2B));
	DSI_WRITE(SEQ_TSP_VSYNC_ON, ARRAY_SIZE(SEQ_TSP_VSYNC_ON));
	DSI_WRITE(SEQ_ERR_FG_ENABLE, ARRAY_SIZE(SEQ_ERR_FG_ENABLE));
	DSI_WRITE(SEQ_ERR_FG_SET, ARRAY_SIZE(SEQ_ERR_FG_SET));
	DSI_WRITE(SEQ_PCD_SET_DET_LOW, ARRAY_SIZE(SEQ_PCD_SET_DET_LOW));
	DSI_WRITE(SEQ_PARTIAL_UPDATE, ARRAY_SIZE(SEQ_PARTIAL_UPDATE));
...
static int s6e3fa9_register_notifier(struct lcd_info *lcd)
{
...
static int s6e3fa9_probe(struct lcd_info *lcd)
{
	int ret = 0;
	...
	ret = s6e3fa9_read_init_info(lcd);
	if (ret < 0)
...
struct dsim_lcd_driver s6e3fa9_mipi_lcd_driver = {
	.name		= "s6e3fa9",
...
__XX_ADD_LCD_DRIVER(s6e3fa9_mipi_lcd_driver);
