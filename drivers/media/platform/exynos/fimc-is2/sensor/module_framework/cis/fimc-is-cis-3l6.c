...
int sensor_3l6_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	...
	if (min_delay - u_delay > 0)
		usleep_range(min_delay - u_delay, min_delay - u_delay);
...
static const struct of_device_id exynos_fimc_is_cis_3l6_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-3l6",
	},
	{},
};
MODULE_DEVICE_TABLE(of, exynos_fimc_is_cis_3l6_match);
...
static struct i2c_driver cis_3l6_driver = {
	.driver = {
		.name	= SENSOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = exynos_fimc_is_cis_3l6_match
	},
	.probe	= cis_3l6_probe,
