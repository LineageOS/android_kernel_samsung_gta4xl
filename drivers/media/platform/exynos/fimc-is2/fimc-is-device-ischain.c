...
int fimc_is_ischain_power(struct fimc_is_device_ischain *device, int on)
{
	...
	if (on) {
		...
		if(core->current_position == SENSOR_POSITION_REAR) {
			sm5713_fled_mode_ctrl(SM5713_FLED_INDEX_1, SM5713_FLED_MODE_PREPARE_FLASH);
			fled_prepared = true;
		}
#endif
		...
		if(core->current_position == SENSOR_POSITION_REAR || fled_prepared) {
			sm5713_fled_mode_ctrl(SM5713_FLED_INDEX_1, SM5713_FLED_MODE_CLOSE_FLASH);
			fled_prepared = false;
		}
#endif
		...
		for (sindex = 0; sindex < FIMC_IS_SENSOR_COUNT; ++sindex) {
			...
			if (position < SENSOR_POSITION_MAX) {
				...
				case SENSOR_POSITION_VIRTUAL:
					...
					break;
				default:
					merr("invalid module positon(V)(%d)", device, position);
					ret = -EINVAL;
					goto p_err;
				}
			}
